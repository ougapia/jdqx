...
[task_local]
#签到领现金
2 0-23/6 * * * jd_cash.js, tag=签到领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "2 0-23/6 * * *" script-path=jd_cash.js,tag=签到领现金

===============Surge=================
签到领现金 = type=cron,cronexp="2 0-23/6 * * *",wake-system=1,timeout=3600,script-path=jd_cash.js

============小火箭=========
签到领现金 = type=cron,script-path=jd_cash.js, cronexpr="2 0-23/6 * * *", timeout=3600, enable=true
 */
const $ = new Env('签到领现金');
...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://raw.githubusercontent.com/yongyuanlin/cast/main/mast/jd_updateCash.json')
  if (!$.authorCode) {
  $.authorCode = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/yongyuanlin/cast@main/mast/jd_updateCash.json') || []
  }
